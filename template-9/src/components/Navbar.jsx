        ...
        <nav className="navbar w-full flex py-6 justify-between items-center">
            ...
            <ul className="list-none sm:flex hidden justify-end items-center flex-1">
                {navLinks.map((ele, index) =>
                 ...
                 ${ele.id == navLinks.length - 1 ? "mr-0" : "mr-10"}`} key={index}>
            ...
            <div className="sm:hidden flex flex-1 justify-end items-center">
                ...
                <div className={`sidebar ${toggle ? "flex" : "hidden"} p-6 bg-black-gradient absolute top-20 right-0
                mx-4 my-2 min-w-[140px] rounded-xl`}>
                    <ul className="list-none flex flex-col justify-end items-center flex-1">
                        {navLinks.map((ele, index) =>
                            <li className={`font-poppins font-normal cursor-pointer text-[16px] text-white 
                 ${ele.id == navLinks.length - 1 ? "mb-0" : "mb-4"}`} key={index}>
                                <a href={ele.id}>{ele.title}</a>
                            </li>)}
                    </ul>
                </div>
